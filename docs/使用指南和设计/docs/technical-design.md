# BR AI Rules V0.4 技术实现文档

## 1. 架构目标

V0.3 起支持本地团队规则源；**V0.4** 在此基础上提供 `source init`（默认 `team.starter-pack` 轻量骨架）、`source check`（独立校验规则源）与 `doctor`（项目内诊断），团队仍可在独立目录或仓库中维护共享规则并在多项目复用。
